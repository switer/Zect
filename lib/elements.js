        ...
        'if': {
            ...
            update: function(next) {
                if (!next) {
                    ...
                    // Inherit from parent scope's properties
                    var $scope = this._scope = new Scope($parent.data, $parent)
                    var protoUpdate = $scope.$update
                    $scope.$update = function () {
                        // the "if" directive is sharing with $scope.data, 
                        // so it need to be updated
                        $scope.$data = $parent.$data
                        protoUpdate.apply($scope, arguments)
                    }
                    var $update = this.$update
                    // Rewrite to $update interface
                    this.$update = function () {
                        $scope.$update()
                        $update.apply(this, arguments)
                    }
                    // $parent is scope context of the directive
                    if($parent) {
                        this.$scope.children.push($scope)
                    }
                    this._compiler = this.$vm.$compile(this._tmpCon, $scope)
                    this._mount()
                }
            },
            unbind: function () {
                this._compiler.$remove().$destroy()
                this._scope.$destroy()
