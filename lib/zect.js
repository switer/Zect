...
function ViewModel(options) {
    ...
    vm.$compile = function (el, scope) {
        ...
        // The uniform interface for a compiled node
        return compiler
    }
    ...
    vm.$destroy = function () {
        ...
        // destroy data observer instance
        $data.$destroy()

        // instance methods/properties
        vm.$el = vm.$get = vm.$set = vm.$refs = vm.$watch = vm.$unwatch = vm.$compile = vm.$component = null

        // private vars
        directives = components = _directives = _components = null

        // marked, destroyed flag
        vm.$destroyed = true
    }
    ...
    function compile (node, scope, isRoot) {
         ...
         */
        var into = true // If "into" is false, it will stop deep compiling.
        var inst
        switch (node.nodeType) {
            case 1:
                /**
                 * Do not parse that node with static flag
                 */
                ...
                /**
                 * Convert those $ns-if, $ns-repeat attribute to scoped element
                 */
    ...
    /**
     * Convert "if", "repeat" from attributed declaration to custom tag
     */
    function compilePseudoDirectiveElement (node) {
        ...
        var scopedNode = document.createElement(matchedAttName)
        node.removeAttribute(matchedAttName)
        node.parentNode && node.parentNode.replaceChild(scopedNode, node)
        scopedNode.appendChild(node)
        scopedNode.setAttribute(valueAttName, attValue)

        /**
         * Compile all scoped directive in the same node recursively
         */
